...
>(
  ...
  return async function runAction(
    ...
    try {
      ...
      // dispatch above enforces exactly what `ActionActor` states — `null` for a
      // public Action, a resolved `Actor` for every other mode — and this is the
      // single point where the runtime union and the declared type are joined.
